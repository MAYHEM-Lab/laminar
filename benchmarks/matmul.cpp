...
double matmul_partition(const int mat_size, int n_partitions, bool timing) {
    ...
    // int n_partitions = 2;    // number of partitions
    ...
    for (int i = 1; i <= n_partitions; i++) {
        add_operand(1, 1, i);  // A / n_partitions
    }
    ...
    for (int i = 1; i <= n_partitions; i++) {
        ...
        subscribe(3, i, 0, 1, i);  // Rows of matrix A (A / n_partitions)
    ...
    // Write matrix A to operands in `n_partitions` chunks
    int min_chunk_size = mat_size / n_partitions;
    int overflow = mat_size % n_partitions;
    ...
    operand op(operand_value, 1);
    write_value(&op.operand_value);
    fire_operand(1, n_partitions, &op);
    value_deep_delete(operand_value);
    ...
    if (!timing) {
        // Show matrix multiplication result
        overflow = mat_size % n_partitions;
        operand result;
        for (unsigned int id = 1; id <= n_partitions; id++) {
            int err = get_result(3, id, &result, 1);
            if (err < 0) {
                std::cout << "Failed to read the result " << std::endl;
            }
...
int main() {
    ...
    for (int i = 0; i < 5; i++) {
        latencies_ms.push_back(matmul_partition(4, 4, true));
    }
